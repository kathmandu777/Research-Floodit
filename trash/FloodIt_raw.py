...
def main():
    global FPSCLOCK, DISPLAYSURF, LOGOIMAGE, SPOTIMAGE, SETTINGSIMAGE, SETTINGSBUTTONIMAGE, RESETBUTTONIMAGE, mode
    ...
    while True:  # main game loop
        ...
        checkForQuit()  # 終了ボタン or Escボタンを押されたときの処理
...
def floodFill(board, oldColor, newColor, x, y):  # 色の変更
...
def countSquare(board, originColor, x, y):  # 先頭の色と連続した色のマスのカウント
    ...
    global continueing_square
    continueing_square += 1

    if x > 0:
        countSquare(board, originColor, x - 1, y)  # on box to the left
    if x < boardWidth - 1:
        countSquare(board, originColor, x + 1, y)  # on box to the right
    if y > 0:
        countSquare(board, originColor, x, y - 1)  # on box to up
    if y < boardHeight - 1:
        countSquare(board, originColor, x, y + 1)  # on box to down
...
def floodAnimation(board, paletteClicked, animationSpeed=25):
    ...
    global continueing_square
    continueing_square = 0
    forCountBoard = copy.deepcopy(board)
    countSquare(forCountBoard, board[0][0], 0, 0)  # 先頭の色が連続しているマスの数を数える
    before_continueing_square = continueing_square

    floodFill(board, board[0][0], paletteClicked, 0, 0)  # マスを選択された色に変換

    continueing_square = 0
    forCountBoard2 = copy.deepcopy(board)
    countSquare(forCountBoard2, paletteClicked, 0, 0)  # 先頭の色が連続しているマスの数を数える
    after_continueing_square = continueing_square

    print("変更されたマスの数"+str(after_continueing_square - before_continueing_square))
...
def generateRandomBoard(width, height, difficulty=MEDIUM):  # 盤面の初期化
...
def drawBoard(board, transparency=255):
    ...
    pygame.draw.rect(tempSurf, BLACK, (left-1, top-1, boxSize *
                                       boardWidth + 1, boxSize * boardHeight + 1), 1)
